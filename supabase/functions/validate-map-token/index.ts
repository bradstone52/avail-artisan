...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[validate-map-token] Token valid, fetching listings`);

    // Build query for listings
    // Required constraints: Status == "Active" (distribution warehouse filtering happens at sync time)
    let query = supabase
      .from("listings")
      .select(`
        id,
    ...
    const safeListings: PublicListing[] = (listings || []).map((l: Record<string, unknown>) => ({
      id: l.id as string,
    ...
    console.log(`[validate-map-token] Returning ${safeListings.length} listings`);
